...
{
    return g_executor->mono_thread_suspend_all_other_threads();
}

void mono_thread_pool_cleanup()
{
    return g_executor->mono_thread_pool_cleanup();
}

void mono_threads_set_shutting_down()
{
    return g_executor->mono_threads_set_shutting_down();
}

void mono_runtime_set_shutting_down()
{
    return g_executor->mono_runtime_set_shutting_down();
}

gboolean mono_domain_finalize(MonoDomain* domain, int timeout)
{
    return g_executor->mono_domain_finalize(domain, timeout);
}

void mono_runtime_cleanup(MonoDomain* domain)
{
    return g_executor->mono_runtime_cleanup(domain);
}

MonoMethodDesc* mono_method_desc_new(const char* name, gboolean include_namespace)
{
    return g_executor->mono_method_desc_new(name, include_namespace);
}

MonoMethod* mono_method_desc_search_in_image(MonoMethodDesc* desc, MonoImage* image)
{
    return g_executor->mono_method_desc_search_in_image(desc, image);
}

void mono_verifier_set_mode(MiniVerifierMode m)
{
    return g_executor->mono_verifier_set_mode(m);
}

void mono_security_set_mode(MonoSecurityMode m)
{
    return g_executor->mono_security_set_mode(m);
}

void mono_add_internal_call(const char* name, gconstpointer method)
{
    return g_executor->mono_add_internal_call(name, method);
}

void mono_jit_cleanup(MonoDomain* domain)
{
    return g_executor->mono_jit_cleanup(domain);
}

MonoDomain* mono_jit_init(const char* file)
{
    return g_executor->mono_jit_init(file);
}

MonoDomain* mono_jit_init_version(const char* file, const char* runtime_version)
{
    return g_executor->mono_jit_init_version(file, runtime_version);
}

int mono_jit_exec(MonoDomain* domain, MonoAssembly* assembly, int argc, char* argv[])
{
    return g_executor->mono_jit_exec(domain, assembly, argc, argv);
}

MonoClass* mono_class_from_name(MonoImage* image, const char* name_space, const char* name)
{
    return g_executor->mono_class_from_name(image, name_space, name);
}

MonoAssembly* mono_domain_assembly_open(MonoDomain* domain, const char* name)
{
    return g_executor->mono_domain_assembly_open(domain, name);
}

MonoDomain* mono_domain_create_appdomain(const char* domainname, const char* configfile)
{
    return g_executor->mono_domain_create_appdomain(domainname, configfile);
}

void mono_domain_unload(MonoDomain* domain)
{
    return g_executor->mono_domain_unload(domain);
}

MonoObject* mono_object_new(MonoDomain* domain, MonoClass* klass)
{
    return g_executor->mono_object_new(domain, klass);
}

void mono_runtime_object_init(MonoObject* this_obj)
{
    return g_executor->mono_runtime_object_init(this_obj);
}

MonoObject* mono_runtime_invoke(MonoMethod* method, void* obj, void** params, MonoObject** exc)
{
    return g_executor->mono_runtime_invoke(method, obj, params, exc);
}

void mono_field_set_value(MonoObject* obj, MonoClassField* field, void* value)
{
    return g_executor->mono_field_set_value(obj, field, value);
}

void mono_field_get_value(MonoObject* obj, MonoClassField* field, void* value)
{
    return g_executor->mono_field_get_value(obj, field, value);
}

int mono_field_get_offset(MonoClassField* field)
{
    return g_executor->mono_field_get_offset(field);
}

MonoClassField* mono_class_get_fields(MonoClass* klass, gpointer* iter)
{
    return g_executor->mono_class_get_fields(klass, iter);
}

MonoMethod* mono_class_get_methods(MonoClass* klass, gpointer* iter)
{
    return g_executor->mono_class_get_methods(klass, iter);
}

MonoDomain* mono_domain_get()
{
    return g_executor->mono_domain_get();
}

MonoDomain* mono_get_root_domain()
{
    return g_executor->mono_get_root_domain();
}

gint32 mono_domain_get_id(MonoDomain* domain)
{
    return g_executor->mono_domain_get_id(domain);
}

void mono_assembly_foreach(GFunc func, gpointer user_data)
{
    return g_executor->mono_assembly_foreach(func, user_data);
}

void mono_image_close(MonoImage* image)
{
    return g_executor->mono_image_close(image);
}

void mono_unity_socket_security_enabled_set(gboolean b)
{
    return g_executor->mono_unity_socket_security_enabled_set(b);
}

const char* mono_image_get_name(MonoImage* image)
{
    return g_executor->mono_image_get_name(image);
}

MonoClass* mono_get_object_class()
{
    return g_executor->mono_get_object_class();
}

void mono_set_commandline_arguments(int i, const char* argv[], const char* s)
{
    return g_executor->mono_set_commandline_arguments(i, argv, s);
}

const char* mono_field_get_name(MonoClassField* field)
{
    return g_executor->mono_field_get_name(field);
}

MonoType* mono_field_get_type(MonoClassField* field)
{
    return g_executor->mono_field_get_type(field);
}

int mono_type_get_type(MonoType* type)
{
    return g_executor->mono_type_get_type(type);
}

const char* mono_method_get_name(MonoMethod* method)
{
    return g_executor->mono_method_get_name(method);
}

MonoImage* mono_assembly_get_image(MonoAssembly* assembly)
{
    return g_executor->mono_assembly_get_image(assembly);
}

MonoClass* mono_method_get_class(MonoMethod* method)
{
    return g_executor->mono_method_get_class(method);
}

MonoClass* mono_object_get_class(MonoObject* obj)
{
    return g_executor->mono_object_get_class(obj);
}

gboolean mono_class_is_valuetype(MonoClass* klass)
{
    return g_executor->mono_class_is_valuetype(klass);
}

guint32 mono_signature_get_param_count(MonoMethodSignature* sig)
{
    return g_executor->mono_signature_get_param_count(sig);
}

char* mono_string_to_utf8(MonoString* string_obj)
{
    return g_executor->mono_string_to_utf8(string_obj);
}

MonoString* mono_string_new_wrapper(const char* text)
{
    return g_executor->mono_string_new_wrapper(text);
}

MonoClass* mono_class_get_parent(MonoClass* klass)
{
    return g_executor->mono_class_get_parent(klass);
}

const char* mono_class_get_namespace(MonoClass* klass)
{
    return g_executor->mono_class_get_namespace(klass);
}

gboolean mono_class_is_subclass_of(MonoClass* klass, MonoClass* klassc, gboolean check_interfaces)
{
    return g_executor->mono_class_is_subclass_of(klass, klassc, check_interfaces);
}

const char* mono_class_get_name(MonoClass* klass)
{
    return g_executor->mono_class_get_name(klass);
}

char* mono_type_get_name(MonoType* type)
{
    return g_executor->mono_type_get_name(type);
}

MonoClass* mono_type_get_class(MonoType* type)
{
    return g_executor->mono_type_get_class(type);
}

MonoException* mono_exception_from_name_msg(MonoImage* image, const char* name_space, const char* name, const char* msg)
{
    return g_executor->mono_exception_from_name_msg(image, name_space, name, msg);
}

void mono_raise_exception(MonoException* ex)
{
    return g_executor->mono_raise_exception(ex);
}

MonoClass* mono_get_exception_class()
{
    return g_executor->mono_get_exception_class();
}

MonoClass* mono_get_array_class()
{
    return g_executor->mono_get_array_class();
}

MonoClass* mono_get_string_class()
{
    return g_executor->mono_get_string_class();
}

MonoClass* mono_get_int32_class()
{
    return g_executor->mono_get_int32_class();
}

MonoArray* mono_array_new(MonoDomain* domain, MonoClass* eclass, guint32 n)
{
    return g_executor->mono_array_new(domain, eclass, n);
}

MonoArray* mono_array_new_full(MonoDomain* domain, MonoClass* array_class, guint32* lengths, guint32* lower_bounds)
{
    return g_executor->mono_array_new_full(domain, array_class, lengths, lower_bounds);
}

MonoClass* mono_array_class_get(MonoClass* eclass, guint32 rank)
{
    return g_executor->mono_array_class_get(eclass, rank);
}

gint32 mono_class_array_element_size(MonoClass* ac)
{
    return g_executor->mono_class_array_element_size(ac);
}

MonoObject* mono_type_get_object(MonoDomain* domain, MonoType* type)
{
    return g_executor->mono_type_get_object(domain, type);
}

MonoThread* mono_thread_attach(MonoDomain* domain)
{
    return g_executor->mono_thread_attach(domain);
}

void mono_thread_detach(MonoThread* thread)
{
    return g_executor->mono_thread_detach(thread);
}

MonoThread* mono_thread_exit()
{
    return g_executor->mono_thread_exit();
}

MonoThread* mono_thread_current()
{
    return g_executor->mono_thread_current();
}

void mono_thread_set_main(MonoThread* thread)
{
    return g_executor->mono_thread_set_main(thread);
}

void mono_set_find_plugin_callback(gconstpointer method)
{
    return g_executor->mono_set_find_plugin_callback(method);
}

void mono_security_enable_core_clr()
{
    return g_executor->mono_security_enable_core_clr();
}

bool mono_security_set_core_clr_platform_callback(MonoCoreClrPlatformCB a)
{
    return g_executor->mono_security_set_core_clr_platform_callback(a);
}

MonoRuntimeUnhandledExceptionPolicy mono_runtime_unhandled_exception_policy_get()
{
    return g_executor->mono_runtime_unhandled_exception_policy_get();
}

void mono_runtime_unhandled_exception_policy_set(MonoRuntimeUnhandledExceptionPolicy policy)
{
    return g_executor->mono_runtime_unhandled_exception_policy_set(policy);
}

MonoClass* mono_class_get_nesting_type(MonoClass* klass)
{
    return g_executor->mono_class_get_nesting_type(klass);
}

MonoVTable* mono_class_vtable(MonoDomain* domain, MonoClass* klass)
{
    return g_executor->mono_class_vtable(domain, klass);
}

MonoReflectionMethod* mono_method_get_object(MonoDomain* domain, MonoMethod* method, MonoClass* refclass)
{
    return g_executor->mono_method_get_object(domain, method, refclass);
}

MonoMethodSignature* mono_method_signature(MonoMethod* method)
{
    return g_executor->mono_method_signature(method);
}

MonoType* mono_signature_get_params(MonoMethodSignature* sig, gpointer* iter)
{
    return g_executor->mono_signature_get_params(sig, iter);
}

MonoType* mono_signature_get_return_type(MonoMethodSignature* sig)
{
    return g_executor->mono_signature_get_return_type(sig);
}

MonoType* mono_class_get_type(MonoClass* klass)
{
    return g_executor->mono_class_get_type(klass);
}

void mono_set_ignore_version_and_key_when_finding_assemblies_already_loaded(gboolean value)
{
    return g_executor->mono_set_ignore_version_and_key_when_finding_assemblies_already_loaded(value);
}

void mono_debug_init(int format)
{
    return g_executor->mono_debug_init(format);
}

void mono_debug_open_image_from_memory(MonoImage* image, const char* raw_contents, int size)
{
    return g_executor->mono_debug_open_image_from_memory(image, raw_contents, size);
}

guint32 mono_field_get_flags(MonoClassField* field)
{
    return g_executor->mono_field_get_flags(field);
}
...
{
    return g_executor->mono_image_open_from_data_full(data, data_len, need_copy, status, ref_only);
}
...
{
    return g_executor->mono_dl_fallback_register(load_func, symbol_func, close_func, user_data);
}

void mono_dl_fallback_unregister(MonoDlFallbackHandler* handler)
{
    return g_executor->mono_dl_fallback_unregister(handler);
}
...
{
    return g_executor->mono_unity_liveness_allocate_struct(filter, max_count, callback, callback_userdata);
}

void mono_unity_liveness_stop_gc_world()
{
    return g_executor->mono_unity_liveness_stop_gc_world();
}

void mono_unity_liveness_finalize(LivenessState* state)
{
    return g_executor->mono_unity_liveness_finalize(state);
}

void mono_unity_liveness_start_gc_world()
{
    return g_executor->mono_unity_liveness_start_gc_world();
}

void mono_unity_liveness_free_struct(LivenessState* state)
{
    return g_executor->mono_unity_liveness_free_struct(state);
}
...
{
    return g_executor->mono_unity_liveness_calculation_begin(filter, max_count, callback, callback_userdata);
}

void mono_unity_liveness_calculation_end(LivenessState* state)
{
    return g_executor->mono_unity_liveness_calculation_end(state);
}

void mono_unity_liveness_calculation_from_root(MonoObject* root, LivenessState* state)
{
    return g_executor->mono_unity_liveness_calculation_from_root(root, state);
}

void mono_unity_liveness_calculation_from_statics(LivenessState* state)
{
    return g_executor->mono_unity_liveness_calculation_from_statics(state);
}

void mono_trace_set_level_string(const char* value)
{
    return g_executor->mono_trace_set_level_string(value);
}

void mono_trace_set_mask_string(char* value)
{
    return g_executor->mono_trace_set_mask_string(value);
}

gint64 mono_gc_get_used_size()
{
    return g_executor->mono_gc_get_used_size();
}

gint64 mono_gc_get_heap_size()
{
    return g_executor->mono_gc_get_heap_size();
}
