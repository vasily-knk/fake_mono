...
{
    return functions()->mono_image_open_from_data_full(data, data_len, need_copy, status, ref_only);
}
...
{
    return functions()->mono_dl_fallback_register(load_func, symbol_func, close_func, user_data);
}


void executor_base::mono_dl_fallback_unregister(MonoDlFallbackHandler* handler)
{
    return functions()->mono_dl_fallback_unregister(handler);
}
...
{
    return functions()->mono_unity_liveness_allocate_struct(filter, max_count, callback, callback_userdata);
}


void executor_base::mono_unity_liveness_stop_gc_world()
{
    return functions()->mono_unity_liveness_stop_gc_world();
}


void executor_base::mono_unity_liveness_finalize(LivenessState* state)
{
    return functions()->mono_unity_liveness_finalize(state);
}


void executor_base::mono_unity_liveness_start_gc_world()
{
    return functions()->mono_unity_liveness_start_gc_world();
}


void executor_base::mono_unity_liveness_free_struct(LivenessState* state)
{
    return functions()->mono_unity_liveness_free_struct(state);
}
...
{
    return functions()->mono_profiler_install_exception(throw_callback, exc_method_leave, clause_callback);
}


void executor_base::mono_profiler_install_thread(MonoProfileThreadFunc start, MonoProfileThreadFunc end)
{
    return functions()->mono_profiler_install_thread(start, end);
}
